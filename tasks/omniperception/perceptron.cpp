...
: thread_pool_(4), detection_queue_(10), decider_(config_path), stop_flag_(false)
{
  // 并行任务，读取并推理 USB 相机图像
  auto parallel_infer =
    [&](io::USBCamera & cam, auto_aim::YOLOV8 & yolov8_parallel, const std::string & config_path) {
      while (true) {
        cv::Mat usb_img;
  ...
  auto_aim::YOLOV8 yolov8_parallel1(config_path, false);
  auto_aim::YOLOV8 yolov8_parallel2(config_path, false);
  auto_aim::YOLOV8 yolov8_parallel3(config_path, false);
  auto_aim::YOLOV8 yolov8_parallel4(config_path, false);

  // 将 4 个相机推理任务加入线程池
  thread_pool_.add_task([&] { parallel_infer(usbcam1, yolov8_parallel1, config_path); });
  thread_pool_.add_task([&] { parallel_infer(usbcam2, yolov8_parallel2, config_path); });
  thread_pool_.add_task([&] { parallel_infer(usbcam3, yolov8_parallel3, config_path); });
  thread_pool_.add_task([&] { parallel_infer(usbcam4, yolov8_parallel4, config_path); });
